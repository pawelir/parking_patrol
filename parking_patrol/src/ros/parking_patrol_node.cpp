...
ParkingPatrolNode::ParkingPatrolNode(
    std::shared_ptr<SpotFinderNode> spot_finder_node)
...
MsgPoint ParkingPatrolNode::msg_point_from_detection(
    const SpotDetectionPose &detection) {
  MsgPoint point;
  point.x = detection.x;
  point.y = detection.y;
  return point;
