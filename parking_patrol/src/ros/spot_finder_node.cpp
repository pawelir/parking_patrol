...
SpotFinderNode::SpotFinderNode() : Node("spot_finder_node")
{
  ...
  sub_odom_ = this->create_subscription<MsgOdom>(
    "/odom", default_qos, std::bind(&SpotFinderNode::odom_cb, this, _1), sub_options);

  sub_laser_scan_ = this->create_subscription<MsgLaserScan>(
    "/scan", sensor_qos, std::bind(&SpotFinderNode::laser_scan_cb, this, _1), sub_options);

  sub_imu_ = this->create_subscription<MsgImu>(
    "/imu", sensor_qos, std::bind(&SpotFinderNode::imu_cb, this, _1), sub_options);

  timer_ = this->create_wall_timer(
    std::chrono::milliseconds(100), std::bind(&SpotFinderNode::timer_callback, this));
...
std::vector<float> SpotFinderNode::select_readings(
  const std::string & side_name, const std::vector<float> & lidar_readings)
{
...
void SpotFinderNode::timer_callback()
{
  ...
  try {
    if (detected_free_spot(laser_readings_left) && !multiple_detection(side::LEFT, position_x)) {
      spot_detections_on_left_.emplace_back(position_x, position_y, orientation);
    }
    if (detected_free_spot(laser_readings_right) && !multiple_detection(side::RIGHT, position_x)) {
      spot_detections_on_right_.emplace_back(position_x, position_y, orientation);
    }
  }
...
bool SpotFinderNode::detected_free_spot(const std::vector<float> & lidar_readings)
{
  if (lidar_readings.size() > 0) {
    return *std::min_element(lidar_readings.begin(), lidar_readings.end()) >= spot_depth_;
  } else
...
bool SpotFinderNode::multiple_detection(const std::string & detection_side, float position_x)
{
  if (detection_side == side::LEFT) {
    if (
      spot_detections_on_left_.size() == 0 ||
  ...
  if (detection_side == side::RIGHT) {
    if (
      spot_detections_on_right_.size() == 0 ||
...
std::unordered_map<std::string, std::vector<SpotDetectionPose>> SpotFinderNode::get_detections()
{
  return std::unordered_map<std::string, std::vector<SpotDetectionPose>>{
    {side::LEFT, spot_detections_on_left_}, {side::RIGHT, spot_detections_on_right_}};
}
